...
export async function runHookAgent(transcript: string): Promise<AgentResult> {
  ...
  try {
    ...
    const response = await openai.responses.create({
      ...
      reasoning: {
        effort: HOOK_CONFIG.reasoning_effort as 'low' | 'medium' | 'high'
      },
      text: {
        verbosity: HOOK_CONFIG.verbosity as 'low' | 'medium' | 'high',
        format: {
          type: 'json_schema',
          name: HOOK_SCHEMA.name,
          schema: HOOK_SCHEMA.schema,
          strict: HOOK_SCHEMA.strict
        }
      }
    });
