// ============================================================================
// TEST: Bridge Agent Only
// Tests the Bridge agent independently before orchestrator
// ============================================================================

import 'dotenv/config';
import { readFileSync, readdirSync } from 'fs';
import { join } from 'path';
import { runBridgeAgent } from './api/agents/bridge';

async function testBridgeAgent() {
  console.log('\n🌉 Testing Bridge Agent...\n');
  ...
  try {
    ...
    // Run Bridge agent
    console.log('🚀 Running Bridge agent...\n');
    const startTime = Date.now();

    const result = await runBridgeAgent(transcript);

    const totalTime = Date.now() - startTime;

    // Display results
    if (result.success) {
      console.log('✅ BRIDGE AGENT SUCCESS!\n');
      ...
      console.log('❌ BRIDGE AGENT FAILED!\n');
