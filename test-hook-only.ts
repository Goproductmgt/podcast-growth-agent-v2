// ============================================================================
// TEST: Hook Agent Only
// Tests the Hook agent independently before orchestrator
// ============================================================================

import 'dotenv/config';
import { readFileSync, readdirSync } from 'fs';
import { join } from 'path';
import { runHookAgent } from './api/agents/hook';

async function testHookAgent() {
  console.log('\n🎯 Testing Hook Agent...\n');
  ...
  try {
    ...
    // Run Hook agent
    console.log('🚀 Running Hook agent...\n');
    const startTime = Date.now();
    
    const result = await runHookAgent(transcript);
    
    const totalTime = Date.now() - startTime;
    
    // Display results
    if (result.success) {
      console.log('✅ HOOK AGENT SUCCESS!\n');
